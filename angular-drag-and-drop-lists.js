        ...
        function($parse,   $timeout,   dndDropEffectWorkaround,   dndDragTypeWorkaround) {
            return function(scope, element, attr) {
                ...
                element.on('drop', function(event) {
                    ...
                    var transferredArray;

                    if (data){
                        try {
                            transferredArray = [].concat(JSON.parse(data));
                        } catch(e) {
                            return stopDragover();
                        }
                    }else if (event.dataTransfer.files){
                        transferredArray = event.dataTransfer.files;
                    }else{
                        return stopDragover();
                    }
                    ...
                    if (attr.dndDrop) {
                        invokeCallback(attr.dndDrop, event, index, transferredArray, onTransferDataReadyCallback);
                    }else{
                        onTransferDataReadyCallback(transferredArray);
                    }
                });
                 ...
                 */
                function invokeCallback(expression, event, index, item, callback) {
